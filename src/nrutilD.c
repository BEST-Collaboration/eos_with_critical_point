...
/* allocate a double 4tensor with range t[nrl..nrh][ncl..nch][ndl..ndh][nwl...nwh] */
{
	...
	t[nrl][ncl][ndl] -= nwl;
	      ...
	      for(i=nrl+1;i<=nrl+nrow-1;i++)t[i]=t[i-1]+ncol;
            for(j=ncl+1;j<=ncl+nrow*ncol-1;j++)t[nrl][j]=t[nrl][j-1]+ndep;
                for(k=ndl+1;k<=ndl+nrow*ncol*ndep-1;k++)t[nrl][ncl][k]=t[nrl][ncl][k-1]+nwid;

	/* return pointer to array of array of pointers to rows */
	return t;
}
